...
class Picking(models.Model):
                        ...
                        for move_line in move_id.move_line_ids:
                            move_line.write(
                                {"internal_ref_lot": pack_lot_name["ref"] if pack_lot_name != False else False,
                                 ...
                                 'categ_1' : nhcl_categ_1.id,
                                 'categ_2' : nhcl_categ_2.id,
                                 'categ_3' : nhcl_categ_3.id,
                                 'categ_4' : nhcl_categ_4.id,
                                 'categ_5' : nhcl_categ_5.id,
                                 'categ_6' : nhcl_categ_6.id,
                                 'categ_7' : nhcl_categ_7.id,
                                 'categ_8' : nhcl_categ_8.id,
                                 'descrip_1' : nhcl_descrip_1,
                                 'descrip_2' : nhcl_descrip_2,
                                 'descrip_3' : nhcl_descrip_3,
                                 'descrip_4' : nhcl_descrip_4,
                                 'descrip_5' : nhcl_descrip_5,
                                 'descrip_6' : nhcl_descrip_6,
                                 'descrip_7' : nhcl_descrip_7,
                                 'descrip_8' : nhcl_descrip_8,
                                 })
                else:
            ...
            for move_line in rec.move_ids_without_package:
                if move_line.dummy_lot_ids:
                    ...
                    if rec.company_type == "same":
                        ...
                        move_line.move_line_ids.lot_id.update({'cost_price': move_line.nhcl_cost_price})
        ...
        # --------------------- EAN-13 Barcode branch ---------------------
            ...
            if product.tracking == 'serial':
                ...
                for lot in lots.lot_id:
                    ...
                    if (total_assigned_qty + self.lot_qty) <= lots.quantity:
                        available_lot = lot
                        break
    ...
    def action_confirm(self):
        ...
        for picking in self:
            if picking.stock_picking_type in ['damage','damage_main','main_damage','return_main']:
                for move in picking.move_ids_without_package:
                    if move.dummy_lot_ids:
                        move.lot_ids = [(6, 0, move.dummy_lot_ids.ids)]
        return res
