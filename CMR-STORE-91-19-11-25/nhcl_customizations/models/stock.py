...
class Picking(models.Model):
    ...
    def _cron_notify_old_backorders(self):
        """
        Find incoming backorders older than 1 month (draft/assigned)
        Post 1 Discuss message per backorder to 'Backorder Alerts' channel.
        Channel auto-syncs members with security group.
        No summary message — only actual records.
        """

        one_month_ago = datetime.now() - timedelta(days=30)
        channel_name = "Backorder Alerts"

        # Step 1: Find eligible backorders
        old_backorders = self.env['stock.picking'].search([
            ('picking_type_code', '=', 'incoming'),
            ('backorder_id', '!=', False),
            ('create_date', '<', one_month_ago),
            ('state', 'not in', ['done', 'cancel']),
        ])
        count_backorders = len(old_backorders)
        print(f"*** Found {count_backorders} old backorders:", old_backorders.mapped('name'))

        # Step 2: Get security group
        group = self.env.ref('nhcl_customizations.group_backorder_notification')
        if not group:
            print("⚠️ Security group 'group_backorder_notification' not found.")
            return

        group_partner_ids = set(group.users.mapped('partner_id.id'))

        # Step 3: Find or create the Discuss channel
        channel = self.env['discuss.channel'].search([('name', '=', channel_name)], limit=1)
        if not channel:
            channel = self.env['discuss.channel'].create({
                'name': channel_name,
                'channel_type': 'group',
                'channel_partner_ids': [(6, 0, list(group_partner_ids))],
            })
            print(f"🆕 Created Discuss channel '{channel_name}'.")

        # Step 4: Sync members with group
        channel_partner_ids = set(channel.channel_partner_ids.ids)
        partners_to_add = group_partner_ids - channel_partner_ids
        partners_to_remove = channel_partner_ids - group_partner_ids

        if partners_to_add:
            channel.write({'channel_partner_ids': [(4, pid) for pid in partners_to_add]})
            print(f"✅ Added {len(partners_to_add)} members to '{channel_name}'.")
        if partners_to_remove:
            channel.write({'channel_partner_ids': [(3, pid) for pid in partners_to_remove]})
            print(f"🧹 Removed {len(partners_to_remove)} members who left the group.")

        # Step 5: Post detailed messages only if records exist
        if count_backorders > 0:
            all_partners = channel.channel_partner_ids - self.env.user.partner_id
            ...
            for picking in old_backorders:
                ...
                msg_body = (
                    ...
                    # f"<b>State:</b> {picking.state.title()}<br/>"
                ...
                # 🔔 Mark message unread for all other users (show badge count)
                self._create_unread_notifications(msg, all_partners)

            print(f"✅ Posted {count_backorders} notifications to '{channel_name}'.")
        else:
            print(f"ℹ️ No old backorders found. Channel '{channel_name}' kept active.")

    def _create_unread_notifications(self, message, partners):
        """Mark given message as unread for given partners (Odoo 17 compatible)."""
                    ...
                    for line_data in pos.get("data")[0]["lines"]:
                        ...
                        if pos_line.get("data"):
                            ...
                            if pos_line_product.get("data"):
                                ...
                                product_type = product_data.get("detailed_type")  # product / consumable / service
                                product_name = product_data.get("name")
                                ser_prod = line["full_product_name"]
                                product_nhcl_id = product_data.get("nhcl_id")
                                # 1. Storable Product → Match using nhcl_id
                                # -----------------------------------------------
                                if product_type == 'product' and product_nhcl_id:
                                    product_id = self.env['product.product'].search([
                                        ('nhcl_id', '=', product_nhcl_id)
                                    ], limit=1)
                                ...
                                if not product_id:
                                    raise ValidationError(
                                        _("Product %s (type: %s) not found in HO", product_name, product_type)
                                    )
                        pack_lot_name = False
                        nhcl_categ_1 = False
                        nhcl_categ_2 = False
                        ...
                        if "pack_lot_ids" in line:
                            ...
                            if pack_lot_name["category_1"]:
                                nhcl_categ_1 = self.env['product.attribute.value'].search([('name','=',pack_lot_name["category_1"][0]["name"]),('attribute_id.name','=','Color')])
                            if pack_lot_name["category_2"]:
                                nhcl_categ_2 = self.env['product.attribute.value'].search([('name','=',pack_lot_name["category_2"][0]["name"]),('attribute_id.name','=','Fit')])
                            if pack_lot_name["category_3"]:
                                nhcl_categ_3 = self.env['product.attribute.value'].search([('name','=',pack_lot_name["category_3"][0]["name"]),('attribute_id.name','=','Brand')])
                            if pack_lot_name["category_4"]:
                                nhcl_categ_4 = self.env['product.attribute.value'].search([('name','=',pack_lot_name["category_4"][0]["name"]),('attribute_id.name','=','Pattern')])
                            if pack_lot_name["category_5"]:
                                nhcl_categ_5 = self.env['product.attribute.value'].search([('name','=',pack_lot_name["category_5"][0]["name"]),('attribute_id.name','=','Border Type')])
                            if pack_lot_name["category_6"]:
                                nhcl_categ_6 = self.env['product.attribute.value'].search([('name','=',pack_lot_name["category_6"][0]["name"]),('attribute_id.name','=','Border Size')], limit=1)
                            if pack_lot_name["category_7"]:
                                nhcl_categ_7 = self.env['product.attribute.value'].search([('name','=',pack_lot_name["category_7"][0]["name"]),('attribute_id.name','=','Size')])
                            if pack_lot_name["category_8"]:
                                nhcl_categ_8 = self.env['product.attribute.value'].search([('name','=',pack_lot_name["category_8"][0]["name"]),('attribute_id.name','=','Design')])
                            ...
                            if pack_lot_name["description_2"]:
                                nhcl_descrip_2 = self.env['product.attribute.value'].search([('name','=',pack_lot_name["description_2"][0]["name"]),('attribute_id.name','=','Range')])
                            if pack_lot_name["description_3"]:
                                nhcl_descrip_3 = self.env['product.attribute.value'].search([('name','=',pack_lot_name["description_3"][0]["name"]),('attribute_id.name','=','Collection')])
                            if pack_lot_name["description_4"]:
                                nhcl_descrip_4 = self.env['product.attribute.value'].search([('name','=',pack_lot_name["description_4"][0]["name"]),('attribute_id.name','=','Fabric')], limit=1)
                            if pack_lot_name["description_5"]:
                                nhcl_descrip_5 = self.env['product.attribute.value'].search([('name','=',pack_lot_name["description_5"][0]["name"]),('attribute_id.name','=','Exclusive')])
                            if pack_lot_name["description_6"]:
                                nhcl_descrip_6 = self.env['product.attribute.value'].search([('name','=',pack_lot_name["description_6"][0]["name"]),('attribute_id.name','=','Print')])
                            if pack_lot_name["description_7"]:
                                nhcl_descrip_7 = self.env['product.attribute.value'].search([('name','=',pack_lot_name["description_7"][0]["name"]),('attribute_id.name','=','Days Ageing')])
                ...
                if product.tracking == 'serial':
                    ...
                    other_qty = sum(
                        ...
                        for ml in self.env['stock.move'].search([
                            ...
                            ('move_picking_type', '=', self.stock_picking_type),
                            ('picking_id.state', 'not in', ['done','cancel']),  # include draft, assigned, confirmed, done
                        ])
                    )
