...
async set_quantity(quantity, keep_price) {
    ...
    if (
        ...
        return this.pos.env.services.popup.add(ErrorPopup, {
                    ...
                    body: _t("Serial-tracked products can only have quantity 1 per line."),
                });
    ...
    // Handle lot-tracked products
        ...
        try {
            ...
            const lot_data = await this.pos.orm.call('stock.lot', 'search_read', [
                [['name', '=', lot_name]],
                ['id', 'name', 'product_id','location_id','product_qty']
            ]);
            ...
            if (lot_data.length > 0) {
                ...
                if (quantity > lot_qty_available) {
                   return this.pos.env.services.popup.add(ErrorPopup, {
                        ...
                        body: _t("Only "+ lot_qty_available + " units are available in Lot " + lot_name),
                    });
                }
            }
        } catch (error) {
