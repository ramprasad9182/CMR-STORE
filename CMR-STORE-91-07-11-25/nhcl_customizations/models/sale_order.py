    ...
    def _onchange_barcode_scanned(self):
        ...
        if self.barcode_scanned:
            ...
            if re.match(gs1_pattern, barcode):
                ...
                if lot.rs_price <= 0.0:
                    raise ValidationError(
                        f"You are not allowed to some of the serial numbers are not done landed cost.")
                ...
                if product.tracking == 'serial':
                    ...
                    lot_qty = self.env['stock.move.line'].sudo().search([
                        ('move_line_picking_type', 'in', ['receipt','return_main']), ('lot_id', '=', lot.id),
                        ('location_dest_id.name', '=', location)])
                    if existing_qty + qty > sum(lots.mapped('quantity')):
            ...
            # EAN-13 Barcode
                ...
                if not product or product.tracking not in ('serial', 'lot'):
                    raise ValidationError(f"Product must be tracked (serial or lot).")
                ...
                if product.tracking == 'serial':
                    ...
                    if not available_lot:
                        raise ValidationError(f"All serials for barcode {ean13_barcode} are already used.")
                ...
                # ---- LOT PRODUCT HANDLING WITH PARTIAL SPLIT ----
                else:
                    if not self.entered_qty or self.entered_qty <= 0:
                        raise ValidationError("Enter a valid quantity for lot tracked product.")
                    ...
                    for quant in lots:
                        ...
                        # Skip already-used lots in current order
                        if lot.name in used_names:
                            continue

                        existing_qty = global_lot_qty(lot, sale_serial_type)
                        available_qty = quant.quantity - existing_qty
                        if available_qty <= 0:
                            continue
                        ...
                        if lot.rs_price <= 0.0:
                            raise ValidationError(f"Lot {lot.name} missing landed cost.")
                    ...
                    # Validation if total qty not met
                    if remaining_qty > 0:
                        raise ValidationError(
                            f"Only {self.entered_qty - remaining_qty} qty available for barcode {ean13_barcode}. "
                            f"Missing {remaining_qty} qty."
                        )

                    # Add newly created lines to order
                    self.order_line = new_lines + existing_order_line_cmds
            ...
            # Custom Serial Barcode
                ...
                location = self.env.ref('stock.stock_location_stock').id
                lots = self.env['stock.quant'].search(['|',
                                                       ('lot_id.ref', '=', prefix), ('lot_id.name', '=', f'{prefix}'),
                                                       ('quantity', '>', 0),('location_id.id','=',location),
                                                       ('company_id', '=', self.company_id.id)
                                                       ])
                if not lots:
                ...
                for lot in lots.lot_id:
                    ...
                    if product.tracking == 'serial':
                        ...
                        location = self.env.ref('stock.stock_location_stock').id
                        lot_qty = self.env['stock.move.line'].sudo().search([
                            ('move_line_picking_type', 'in', ['receipt','return_main']),
                            ('lot_id', '=', lot.id),
                            ('location_dest_id.id', '=', location)])
                        if existing_qty + self.entered_qty <= sum(lots.mapped('quantity')):
                            selected_lot = lot
                            qty = self.entered_qty
                            break
                if not selected_lot:
