...
set_quantity(quantity, keep_price) {
        // If product is tracked by serial, force qty = 1
        if (this.product.nhcl_product_type === 'unbranded' && this.product.tracking === 'serial' && quantity > 1) {
            quantity = 1;
        }
        else if (this.product.nhcl_product_type === 'branded' && this.product.tracking === 'serial' && quantity > 1) {
            quantity = 1;
        }
         else if (this.product.nhcl_product_type === 'branded' && this.product.tracking === 'lot' && quantity > 1) {
            quantity = 1;
        }
  ...
  const reward = this.pos.reward_by_id[this.reward_id];
       if (this.reward_product_id){
        let reward_product = this.order.get_orderlines().find(
            (line) => line.product.id === this.reward_product_id);
     ...
     else if (this.reward_id && reward.reward_type == 'discount_on_product'){
