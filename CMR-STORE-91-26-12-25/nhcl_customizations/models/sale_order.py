...
class SaleOrder(models.Model):
    ...
    transfer_type = fields.Selection([('regular', 'Regular'), ('damage', 'Damage')],
    ...
    def _onchange_barcode_scanned(self):
        ...
        if self.barcode_scanned:
            ...
            # GS1 BARCODE
            # ----------------------------------------------------------------------------
            if re.match(gs1_pattern, barcode) and self.transfer_type == 'regular':
            ...
            if re.match(gs1_pattern, barcode) and self.transfer_type == 'damage':
                product_barcode, scanned_number = re.match(gs1_pattern, barcode).groups()
                product = search_product('barcode', product_barcode)
                if not product:
                    raise ValidationError(f"No product found with barcode {product_barcode}.")
                ...
                if product.tracking == 'serial':
                    ...
                    existing_orders = global_damage_serial_used_orders(scanned_number, sale_serial_type)
                    if existing_orders:
                        raise ValidationError(
                            f"Serial {scanned_number} already used in: {', '.join(set(existing_orders))}")
            ...
            # ----------------------------------------------------------------------------
            elif re.match(custom_serial_pattern, barcode) and self.transfer_type == 'regular':
                ...
                for lot in lots.lot_id:
                    ...
                    # ---------------- LOT (MERGING) ----------------
                        ...
                        break
                if not selected_lot:
                    raise ValidationError(f"No available lot found for {prefix}")
            ...
            elif re.match(custom_serial_pattern, barcode) and self.transfer_type == 'damage':
                prefix = re.match(custom_serial_pattern, barcode).group(1)
                Quant = self.env['stock.quant']
                lots = Quant.search([
                    ('quantity', '>', 0),
