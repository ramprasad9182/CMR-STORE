...
class Inventory(models.Model):
    ...
    nhcl_replication_status = fields.Boolean(string="Update status")
    ...
    #             # -------------------------------------------------
    #             ho_id = record.env['nhcl.ho.store.master'].search([
    #                 ('nhcl_store_type', '=', 'ho'),
    #                 ('nhcl_active', '=', True)
    #             ], limit=1)
    #
    ...
    #                 log_vals.update({
    #                     'nhcl_status': 'failure',
    #                     'nhcl_details_status': 'HO configuration not found',
    #                 })
    ...
    #             # -------------------------------------------------
    #             company_search_url = f"http://{ho_ip}:{ho_port}/api/res.company/search"
    #             company_domain = [('name', '=', record.company_id.name)]
    #             company_url = f"{company_search_url}?domain={company_domain}"
    #
    #             company_response = requests.get(
    #                 company_url,
    #                 headers=headers_source
    #             ).json()
    #             if not company_response.get("data"):
    #                 log_vals.update({
    #                     'nhcl_status': 'failure',
    #                     'nhcl_details_status': 'Company not found in HO',
    #                 })
    #                 record.env['nhcl.audit.replication.log'].create(log_vals)
    #                 continue
    #
    #             store_company_id = company_response["data"][0]['id']
    #
    #             # Search Audit Plan in HO
    #             # -------------------------------------------------
    #             audit_search_url = f"http://{ho_ip}:{ho_port}/api/stock.inventory/search"
    #             audit_domain = [
    #                 ('name', '=', record.name),
    #                 ('company_id', '=', store_company_id)
    #             ]
    #             store_audit_url = f"{audit_search_url}?domain={audit_domain}"
    #
    #             audit_response = requests.get(
    #                 store_audit_url,
    #                 headers=headers_source
    #             ).json()
    #             if not audit_response.get("data"):
    #                 log_vals.update({
    #                     'nhcl_status': 'failure',
    #                     'nhcl_details_status': 'Audit Plan not found in HO',
    #                 })
    #                 record.env['nhcl.audit.replication.log'].create(log_vals)
    #                 continue
    #
    #             store_audit_plan_id = audit_response["data"][0]['id']
    #
    #             # Update Audit Plan in HO
    #             # -------------------------------------------------
    #             update_url = f"http://{ho_ip}:{ho_port}/api/stock.inventory/{store_audit_plan_id}"
    #
    #             update_vals = {
    #                 'state': record.state
    #             }
    ...
    #
    #             _logger.info(
    #                 f"Audit Plan {record.name} successfully synced to HO"
    #             )
    #
    #         except Exception as e:
    #             _logger.exception("Error while updating Audit Plan to HO")
