...
class AccountMove(models.Model):
    ...
    def get_store_inventory_to_ho(self):
        ...
        for ho in ho_ids:
            ...
            for inventory in inventories:
                try:
                    ...
                    # -------------------------------------------------
                    company_url = f"http://{ho_ip}:{ho_port}/api/res.company/search"
                    company_domain = [('name', '=', inventory.company_id.name)]
                    company_res = requests.get(
                        f"{company_url}?domain={company_domain}",
                        headers=headers
                    ).json()

                    company_data = company_res.get("data")
                    if not company_data:
                        ho.create_cmr_transaction_server_replication_log(
                            "failure",
                            f"Company not found for Inventory {inventory.name}"
                        )
                        continue
                    ...
                    # -------------------------------------------------
                    # Check Existing Inventory in HO
                    # -------------------------------------------------
                    inventory_search = f"http://{ho_ip}:{ho_port}/api/stock.inventory/search"
                    inv_domain = [
                        ('name', '=', inventory.name),
                        ('company_id', '=', company_id)
                    ]
                    inv_exists = requests.get(
                        f"{inventory_search}?domain={inv_domain}",
                        headers=headers
                    ).json().get("data")

                    if inv_exists:
                        inventory.nhcl_replication_status = True
                        ho.create_cmr_transaction_server_replication_log(
                            "failure",
                            f"Inventory {inventory.name} already exists in HO"
                        )
                        continue

                    # -------------------------------------------------
                    # Prepare Inventory Lines
                    # -------------------------------------------------
                    line_vals = []
                    for line in inventory.inventory_line_ids:
                        ...
                        # product_data = requests.get(
                        #     f"{product_url}?domain={product_domain}",
                        #     headers=headers
                        # ).json().get("data")
                        product_url = f"http://{ho_ip}:{ho_port}/api/stock.lot/search"
                        product_domain = [('name', '=', line.prod_lot_id.name)]
                        product_data = requests.get(
                            f"{product_url}?domain={product_domain}",
                            headers=headers
                        ).json().get("data")
                        ...
                        # Location
                        location_url = f"http://{ho_ip}:{ho_port}/api/stock.location/search"
                        location_domain = [('complete_name', '=', line.location_id.complete_name)]
                        location_data = requests.get(
                            f"{location_url}?domain={location_domain}",
                            headers=headers
                        ).json().get("data")
                        ...
                        if line.prod_lot_id:
                            ...
                            lot_data = requests.get(
                                f"{lot_url}?domain={lot_domain}",
                                headers=headers
                            ).json().get("data")
                        ...
                        line_vals.append((0, 0, {
                            "product_id": product_data[0]['id'],
                            "product_uom_id": product_data[0]['uom_id'][0],
                    ...
                    if not line_vals:
                        ho.create_cmr_transaction_server_replication_log(
                            "failure",
                            f"No valid inventory lines for {inventory.name}"
                        )
                        continue

                    # -------------------------------------------------
                    # Create Inventory in HO
                    # -------------------------------------------------
                    inventory_vals = {
                        "name": inventory.name,
                        "date": inventory.date.strftime("%Y-%m-%d %H:%M:%S"),
                        "company_id": company_id,
                        "state": "done",
                        "inventory_line_ids": line_vals,
                        "is_inventory_count": True,
                        "nhcl_store_inventory": True
                    }

                    create_url = f"http://{ho_ip}:{ho_port}/api/stock.inventory/create"
                    ...
                    if response.get("success"):
                        ...
                        ho.create_cmr_transaction_server_replication_log(
                            "success",
                            f"Inventory {inventory.name} replicated successfully"
                        )
                        ho.create_cmr_transaction_replication_log(
                            response.get("object_name"),
                            inventory.id,
                            200,
                            'add',
                            'success',
                            f"Inventory {inventory.name} created in HO"
                        )
                    else:
                        ho.create_cmr_transaction_server_replication_log(
                            "failure",
                    ...
                    ho.create_cmr_transaction_server_replication_log(
                        "failure",
                        f"Inventory {inventory.name} API Error: {e}"
                    )
