...
patch(Order.prototype, {
    removeOrderline(line) {
        let res = super.removeOrderline(...arguments)
        console.log('==> remove order line', line)
        this._updateRewards();
        return res
    },
       ...
       if (partner && credit_amount > 0) {
            ...
            if (credit_methods.length > 0) {
                const credit_method = credit_methods[0];
                    const newPaymentline = new Payment({ env: this.env },
                        {
                            order: this,
...
//    }
    ...
    _applyReward(reward, coupon_id, args) {
        ...
        console.log('==> _applyReward', result, reward.reward_type )
        if (result === true && reward.reward_type === "product") {
            ...
            console.log('==> rewardProductId', rewardProductId)
            if (rewardProductId) {
