...
patch(RewardButton.prototype, {
    _getPotentialRewards() {
        ...
        const potentialFreeProductRewards =
            this.pos.getPotentialFreeProductRewards();
    ...
    hasClaimableRewards() {
        ...
        if (this._getPotentialRewards().length > 0) {
            if (
                this._getPotentialRewards().filter(
                    ({ reward }) =>
    ...
    auto_apply_rewards() {
        ...
        const highestDiscountRewards = Object.values(rewardsList)
            .map((programRewards) => {
                const filteredRewards = programRewards.filter(
                    (reward) =>
                        !reward.reward.discount_max_amount ||
                        reward.reward.discount_max_amount <= 0
                );
                console.log("filtered", filteredRewards);
                if (filteredRewards.length === 0) return null; // If no valid rewards, return null

                const maxDiscountReward = filteredRewards.reduce(
                    (max, current) => {
                        return current.reward.discount > max.reward.discount
                            ? current
                            : max;
        ...
        for (var i = 0, l = highestDiscountRewards.length; i < l; ++i) {
            ...
            let potentialQty =
                highestDiscountRewards[i]["item"]["potentialQty"];
    ...
    async _applyReward(reward, coupon_id, potentialQty) {
        ...
        if (
            ...
            console.log('==> this is reward ', productId )
        ...
        console.log('==> inside else if',  args["product"], reward.discount_product_id[0])
            ...
            console.log('==> test',rewardProductId, rewardProductName)
            order.get_orderlines().forEach(line => {
                   console.log('==> ',line.product, line.product.id === rewardProductId)
