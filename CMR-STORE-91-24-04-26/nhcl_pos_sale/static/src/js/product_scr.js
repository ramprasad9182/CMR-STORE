...
//                        ['location_id', '=', 8]
                    ...
                    try {
                        const brander_serial_nos = await this.orm.call('stock.lot', 'search_read', [domain,['id', 'name', 'ref', 'rs_price',"mr_price","is_under_plan",'product_qty', 'type_product', 'location_id']]);
...
//        );
    ...
    async action_move_product_damage_to_main() {
        ...
        const barcode = payload.newArray[0].text;
        const lot_qty = payload.newArray[0].qty;
        ...
        if (picking_name) {
            ...
            await this.env.services.popup.add(ConfirmPopup, {
                ...
                body: _t("Transfer is not created for %s.", barcode),
            });
        }
    },
    ...
    async customer() {
        ...
        if (orderlines.length > 1) {
            ...
            default_badge = orderlines.filter((line) => line.cid !== selectedOrderline.cid).at(-1).badge;
        }
...
//            const total = this.currentOrder.get_total_with_tax() + this.currentOrder.get_rounding_applied();
            const total = this.currentOrder.get_custom_totalwithtax();
