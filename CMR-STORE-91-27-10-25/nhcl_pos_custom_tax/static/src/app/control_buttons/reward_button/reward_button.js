...
const rewards = this._getPotentialRewards().filter(({ reward }) => reward.discount_max_amount > 0 || reward.reward_type == 'product' ||
        ...
        reward.reward_type === "discount_on_product");          if (rewards.length >= 1) {
            ...
            console.log('current',current.reward.product_price)
             console.log('max',max.reward.product_price)
            if(current.reward.reward_type =='discount_on_product'){
 let index = 0
 let price = 0
 let amount_total = 0
 ...
 for (const rule of current.reward.program_id.rules){

                 if (amount_total >= rule.minimum_amount){
                     price = [...current.reward.program_id.rewards].sort((a, b) => a.id - b.id)[index].product_price
                      index +=1
                 }
                 }
            return price === current.reward.product_price ? current : max; // Assuming `discount` exists
