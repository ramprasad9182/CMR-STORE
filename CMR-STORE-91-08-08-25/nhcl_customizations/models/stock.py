...
class Picking(models.Model):
    ...
    def _onchange_verify_barcode(self):
        ...
        if self.verify_barcode:
            ...
            if re.match(gs1_pattern, barcode):
                ...
                matched_line = self.move_line_ids_without_package.filtered(lambda x: x.lot_name == code)
                if not matched_line:
                ...
                matched_line = self.move_line_ids_without_package.filtered(
                    lambda x: x.lot_name == code and x.type_product == 'un_brand')
                if not matched_line:
    ...
    #             auto_generate_seq_rec.nhcl_next_number)
    #     else:
    ...
    def validate_related_po_receipt(self):
        for picking in self:
            if picking.picking_type_id.code == 'outgoing' and picking.sale_id:
                ...
                related_po = self.env['purchase.order'].search(
                    [('origin', '=', sale_order.name), ('company_id', '!=', picking.company_id.id)], limit=1)
                # If a related Purchase Order is found
                if related_po:
                    # Get the related receipt (incoming picking)
                    related_receipt_picking = self.env['stock.picking'].search([
                        ('purchase_id', '=', related_po.id), ('picking_type_id.code', '=', 'incoming'),
                        ('state', '!=', 'done')], limit=1)
    ...
    def _onchange_stock_barcode(self):
        ...
        if re.match(gs1_pattern, barcode):
            ...
            if product.tracking == 'lot':
                ...
                lot = self.env['stock.lot'].search(
                    [('product_id', '=', product.id), ('name', '=', lot_or_serial), ('product_qty', '>', 0)], limit=1)
                if not lot:
                    raise ValidationError(f'No lot found with number: {lot_or_serial}')
        ...
        # --------------------- EAN-13 Barcode branch ---------------------
            ...
            if product.tracking == 'serial':
                ...
                for lot in lots.lot_id:
                    # Quantity already assigned in this picking
                    current_qty = sum(
                        ml.product_uom_qty for ml in self.move_ids_without_package
                        if lot.id in ml.dummy_lot_ids.ids
                    )
                    # Quantity already used in other open pickings of the same type
                    other_qty = sum(
                        ml.product_uom_qty for ml in self.env['stock.move'].search([
                            ('dummy_lot_ids.id', '=', lot.id),('move_picking_type','=',self.stock_picking_type),
                            ('picking_id.state', 'not in', ['cancel', 'done']),
                        ])
                    )

                    total_assigned_qty = current_qty + other_qty
                    if (total_assigned_qty + self.lot_qty) <= lot.product_qty:
                        available_lot = lot
                        break

                if not available_lot:
                    raise ValidationError(f"No available lot with enough quantity for barcode {ean13_barcode}.")
                create_move_line(product, available_lot, self.lot_qty)

        # --------------------- Custom Serial branch ---------------------
        elif re.match(custom_serial_pattern, barcode):
            StockLot = self.env['stock.lot']
            # STEP 1: Search for lot by ref = scanned barcode
            lots_with_ref = StockLot.search(
                ['|', ('ref', '=', barcode), ('name', '=', barcode), ('company_id', '=', self.company_id.id), ('product_qty', '>', 0)])
            ...
            used_names |= set(self.env['stock.picking'].search([
                ('picking_type_code', 'in', ['internal', 'outgoing']), ('state', '!=', 'cancel'),
            ...
            for lot in all_matching_lots:
                if product.tracking == 'serial':
                    ...
                    existing_qty = sum(l.product_uom_qty for l in self.move_ids_without_package if
                                       l.dummy_lot_ids and lot.id in l.dummy_lot_ids.ids)
                    if (existing_qty + self.lot_qty) <= lot.product_qty:
                        available_lot = lot
                        qty = self.lot_qty
                        break
            if not available_lot:
    ...
    def action_confirm(self):
        for picking in self:
            if picking.picking_type_id.stock_picking_type == 'return':
                if not all([
...
class StockMove(models.Model):
    ...
    def auto_generate_serial_numbers(self):
        for move in self:
            if move.product_id.tracking == 'serial' and move.picking_id.picking_type_id.code == 'incoming' and move.picking_id.stock_type == 'ho_operation':
                ...
                if auto_generate_seq:
                    Form(self.env['stock.assign.serial'].with_context(
                        ...
                        default_next_serial_number=self.env['stock.lot']._get_next_serial(picking_receipt.company_id,
                                                                                          move.product_id),
                    ...
                    if move.move_line_ids:
                        if move.move_line_ids[-1].lot_id or move.move_line_ids[-1].lot_name:
                            ...
                            if len(serial_no) > 1:
                                auto_generate_seq.nhcl_next_number = int(serial_no[1])
        return True

    def _action_done(self, cancel_backorder=False):
        for move in self:
            if move.product_id.tracking == 'serial' and move.picking_id.picking_type_id.code == 'incoming':
                if int(move.quantity) == len(move.move_line_ids.filtered(lambda x: x.lot_name == False)):
    ...
    def _prepare_move_line_vals(self, quantity=None, reserved_quant=None):
        ...
        if reserved_quant and self.sale_line_id.lot_ids:
            vals["lot_id"] = self.sale_line_id.lot_ids.id
