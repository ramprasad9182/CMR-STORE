...
//                    options.merge = is_merge;
    ...
    if (credit_details && credit_details.length >= 1) {
        ...
        const { confirmed, payload: selectedCredit } = await this.popup.add(SelectionPopup, {
            title: _t("Select Credit"),
            list: creditDetailsList,
        });

        if (!confirmed || !selectedCredit) {
            return;
        }

        // Assign selected credit data to current order
        this.currentOrder.credit_note_amount = selectedCredit.remaining_amount;
        this.currentOrder.credit_id = selectedCredit.id;
