...
class Picking(models.Model):
    ...
    def default_get(self, fields):
        ...
        if self.env.context.get('default_stock_picking_type') == 'exchange':
...
class StockMove(models.Model):
    ...
    def write(self, vals):
        for rec in self:
            qty = vals.get('nhcl_move_qty', rec.nhcl_move_qty)
            if qty and qty > rec.quantity:
                raise ValidationError(
                    "Entered quantity cannot be greater than the Purchase quantity."
                )

            if 'nhcl_move_qty' in vals:
        ...
        for move_line in self:
            if move_line.lot_id:
                ...
                if 'nhcl_purchase_indent_reference' in vals:
                    lot_values['nhcl_purchase_indent_reference'] = vals['nhcl_purchase_indent_reference']
                if lot_values:
