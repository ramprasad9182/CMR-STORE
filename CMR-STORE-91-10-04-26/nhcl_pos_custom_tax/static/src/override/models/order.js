...
patch(Order.prototype, {
   ...
   _isRewardProductPartOfRuleSerial(reward, product,currentline) {
        ...
        let totalProductQty = this.get_orderlines().filter((line) => product.has(line.product.id)).reduce((sum, line) => sum + line.get_quantity(), 0);
     ...
     if (currentline.pack_lot_lines){
        ...
        reward.program_id.rules[0].type_filter === 'grc'
    ) {
        totalProductQty = 0;

        for (const discount_line of this.orderlines.filter(
            line => product.has(line.product.id)
        )) {
            if (!discount_line.pack_lot_lines) {
                continue;
            }

            let discountLotName;
            discount_line.pack_lot_lines.forEach(disc_pack => {
                discountLotName = disc_pack.lot_name;
            });

            const discStockLot = this.pos.stock_lots_by_name[discountLotName];
            const discount_lotId = discStockLot
                ? discStockLot.stockLot.id
                : false;

            const validDiscountLot = reward.program_id.rules.filter(rule =>
                rule.type_filter === 'grc' &&
                rule.serial_ids.has(discount_lotId)
            ).length > 0;
...
//                      currentline.set_discount_minimum_amount(rule.minimum_amount)
                 }
                ...
                for (let reward of program.rewards) {
                    ...
                    if (totalProductQty> 0){
                        for (let i = 0; i < program.rules.length; i++) {
                            ...
                            if (totalProductQty >= program.rules[i].minimum_qty){
                                enable_discount = true
...
//    Stop
    ...
    _getRewardLineValuesDiscount(args) {
        ...
        discountable += -fix_discount_amount;
...
//                 Stop
    ...
    let cheapestLine = this._getCheapestLine();
    ...
    let eligibleLines = this.get_orderlines().filter(line => line.quantity > 0);
    ...
    // 3️⃣ Find cheapest line (fallback safe)
    if (eligibleLines.length && (program.rules.filter(rule => rule.minimum_qty <= eligibleLines.length)).length>0) {
        cheapestLine = eligibleLines.reduce((prev, curr) =>
            prev.getComboTotalPriceWithoutTax() <
            curr.getComboTotalPriceWithoutTax()
                ? prev
                : curr
        );
    } else {
    ...
    // 4️⃣ Tax grouping
    const taxKey = cheapestLine.get_taxes().map(t => t.id).join(",");
