...
class Picking(models.Model):
        ...
        for rec in self:
            ...
            if rec.stock_picking_type == 'receipt' and rec.picking_type_code == 'incoming' and rec.stock_type != 'data_import':
                ...
                assigned_serials = set(
                    rec.move_line_ids_without_package.filtered(lambda x: x.lot_name).mapped('lot_name'))
    ...
    @api.model
    def transfer_damage_main(self, barcode, lot_qty):
        context = {
            ...
            'default_lot_qty': lot_qty,
        }
        self = self.with_context(context).create({})
        if self:
            self.stock_barcode = barcode
    ...
    @api.model
    def transfer_return_main(self, barcode, lot_qty):
        context = {
            'restricted_picking_type_code': 'incoming',
...
class StockPickingBarcode(models.Model):
    ...
    def create(self, vals_list):
        ...
        return super(StockPickingBarcode, self).create(vals_list)
