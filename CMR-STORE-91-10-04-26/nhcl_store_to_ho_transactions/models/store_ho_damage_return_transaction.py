                    ...
                    try:
                        for order in self:
                            if order.stock_picking_type == "exchange":
                                ...
                                dest_location_url = f"http://{ho_ip}:{ho_port}/api/stock.location/search?domain=[('complete_name','=', '{order.location_dest_id.complete_name}'),('active','!=',False)]&fields=['name','id']"
                            ...
                            for line in order.move_line_ids_without_package:
                                ...
                                if line and line.lot_id:
                                    ...
                                    if stock_lot_id:
                                        ...
                                        ho.create_cmr_transaction_replication_log('stock.picking', self.id, 200,
                                                                                           'add', 'failure',
                                                                                           f"{self.name, line.lot_id.name}Lot Not found")
                                if line and lot:
