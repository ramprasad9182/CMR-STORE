...
class Picking(models.Model):
    ...
    def stock_disc_pos_exchange(self):
        ...
        for picking in self:
            for move in picking.move_ids_without_package:
                # Skip if no discount mapping data
                if not move.nhcl_disc_lines:
                    continue
                # Safely parse mapping list (product IDs / references)
                try:
                ...
                if not disc_product_list:
                    continue
                # Normalize values inside mapping list
                disc_product_list = [int(i) for i in disc_product_list if str(i).isdigit()]
                # Actual discount value comes from subtotal
                discount_value = move.nhcl_rsp
                if not discount_value:
                    continue
                # Find matching moves based on product id
                matching_moves = picking.move_ids_without_package.filtered(
                    lambda m: m != move and m.product_id.nhcl_id in disc_product_list)
                if not matching_moves:
                    continue
                matching_count = len(matching_moves)
                share = discount_value / matching_count  # equal split
                # Apply split discount to each item move
                for matched_move in matching_moves:
                    # Adjust subtotal
                    new_subtotal = matched_move.nhcl_rsp + share
                    matched_move.update({
                        'nhcl_rsp': new_subtotal,
                    })
    ...
    def _onchange_stock_barcode(self):
        ...
        # ====================== EAN-13 ======================
            ...
            # Lot
                ...
                for quant in lots:
                    lot = quant.lot_id
                    try:
                        _merge_or_create_lot_line(product, lot, self.lot_qty)
                        break
                    except ValidationError:
                        continue
                else:
                    raise ValidationError("Not enough quantity available in any lot.")
        ...
        # ====================== CUSTOM R-SERIAL ======================
            ...
            lot = lots_found[0].lot_id

            if product.tracking == 'serial':
                _merge_or_create_lot_line(product, lot, 1)
            else:
                if not self.lot_qty or self.lot_qty <= 0:
...
class StockMove(models.Model):
    ...
    nhcl_on_cart_discount_value = fields.Float(string="On Cart Disc Value",copy=False)
