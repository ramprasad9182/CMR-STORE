...
console.log('discount_line_product_id',reward.discount_line_product_id)
    ...
    _getDiscountableOnSpecific(reward) {
     ...
     _getRewardLineValuesDiscountProduct(args) {

    const reward = args["reward"];

    const product = this.pos.db.get_product_by_id(reward.discount_product_id[0]);



    if (!product) {

        return _t("Discount product not found.");
    ...
    if (product.taxes_id.length >= 2) {
        ...
        for (let i = 0; i < product.taxes_id.length; i++) {
            ...
            const taxBracket = this.pos.taxes_by_id[product.taxes_id[i]];

            if (product.lst_price >= taxBracket.min_amount && product.lst_price <= taxBracket.max_amount) {

                taxes_ids = [product.taxes_id[i]];
    ...
    // For "discount_on_product", you usually apply it once

    const quantity = 1;

    const price = reward.product_price || 0;

    const cost = reward.clear_wallet

        ? this._getRealCouponPoints(args["coupon_id"])
...
//            reward_product_id: product.id,
 ...
 _getRewardLineValues(args) {
        ...
        return this._getRewardLineValuesDiscountProduct(args);
